...
def generate_ai_response(safe_message: str) -> tuple[str, list[dict]]:
    """Executes the RAG pipeline and returns the reply + XAI source citations."""
    history = memory_manager.get_recent_history()

    # Pre-expand follow-ups into standalone questions before hitting the chain
    ...
    rag_result = qa_chain.invoke({
        ...
        "chat_history": [],  # Empty: condensation is already done above
    })

    ai_reply = rag_result["answer"]
    retrieved_docs = rag_result.get("source_documents", [])

    # XAI: deduplicated source metadata + snippet
    formatted_sources = []
    seen = set()
    ...
    # Save original message (not expanded form) so history stays readable
